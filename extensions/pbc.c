...
size_t pbc_nearest_neighbor(double position[3], double positions[][3],
                            size_t length, double cell[3][3], int pbc[3],
                            double *out) {
  double pbc_bounds[3];
  for (int i = 0; i < 3; i++) {
    if (pbc[i]) {
...
static PyMethodDef method_def[] = {
    {"pbc_nearest_neighbor", py_pbc_nearest_neighbor, METH_VARARGS,
     "Returns the distance from and index of the nearest neighbor."},
    {NULL, NULL, 0, NULL},
};
...
static struct PyModuleDef module_def = {
    ...
    "_pbc",
    "Module `_pbc` provides optimized C functions for operations with periodic "
    "boundary conditions.",
    -1,
    method_def,
};
...
static PyObject *py_pbc_nearest_neighbor(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "OOOO", &py_position, &py_positions, &py_cell,
                        &py_pbc)) {
    return NULL;
  }
  ...
  size_t index = 0;
  index =
      pbc_nearest_neighbor(position, positions, length, cell, pbc, &distance);
